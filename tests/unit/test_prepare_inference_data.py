...
class TestPrepareInferenceBatch:
    ...
    def test_prepare_inference_batch_returns_dataloader(
        self, mock_from_numpy, sample_dataframe, mock_scaler, mock_device
    ):
    ...
    def test_prepare_inference_batch_raises_error_for_empty_dataframe(
        self, mock_scaler, mock_device
    ):
        """Verifica se levanta erro para DataFrame vazio."""
        empty_df = pd.DataFrame()
        with pytest.raises(
            ValueError, match="DataFrame de features não pode estar vazio"
        ):
            prepare_inference_batch(empty_df, mock_scaler, mock_device)

    def test_prepare_inference_batch_raises_error_for_none_scaler(
        self, sample_dataframe, mock_device
    ):
    ...
    def test_prepare_inference_batch_calls_scaler_transform(
        self, mock_logger, mock_from_numpy, sample_dataframe, mock_scaler, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.from_numpy")
    @patch("src.inference.prepare_inference_data.logger")
    def test_prepare_inference_batch_creates_tensor_correctly(
        self, mock_logger, mock_from_numpy, sample_dataframe, mock_scaler, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.from_numpy")
    @patch("src.inference.prepare_inference_data.logger")
    def test_prepare_inference_batch_handles_sparse_matrix(
        self, mock_logger, mock_from_numpy, sample_dataframe, mock_device
    ):
        ...
        result = prepare_inference_batch(
            sample_dataframe.head(2), mock_scaler, mock_device
        )
        assert isinstance(result, DataLoader)
    ...
    @patch("src.inference.prepare_inference_data.torch.from_numpy")
    @patch("src.inference.prepare_inference_data.logger")
    def test_prepare_inference_batch_warns_high_zero_ratio(
        self, mock_logger, mock_from_numpy, sample_dataframe, mock_scaler, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.from_numpy")
    @patch("src.inference.prepare_inference_data.logger")
    def test_prepare_inference_batch_logs_info_messages(
        self, mock_logger, mock_from_numpy, sample_dataframe, mock_scaler, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.from_numpy")
    def test_prepare_inference_batch_uses_correct_batch_size(
        self, mock_from_numpy, sample_dataframe, mock_scaler, mock_device
    ):
        ...
        batch_size = 2
        result = prepare_inference_batch(
            sample_dataframe, mock_scaler, mock_device, batch_size=batch_size
        )
        assert result.batch_size == batch_size

    @patch("src.inference.prepare_inference_data.torch.from_numpy")
    @patch("src.inference.prepare_inference_data.DataLoader")
    def test_prepare_inference_batch_no_shuffle(
        self,
    ...
    @patch("src.inference.prepare_inference_data.logger")
    def test_prepare_inference_batch_handles_exceptions(
        self, mock_logger, sample_dataframe, mock_scaler, mock_device
    ):
...
class TestRunInference:
    ...
    def test_run_inference_returns_tuple_of_arrays(
        self, mock_sigmoid, mock_dataloader, mock_model, mock_device
    ):
    ...
    def test_run_inference_raises_error_for_none_model(
        self, mock_dataloader, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.sigmoid")
    def test_run_inference_applies_sigmoid_to_logits(
        self, mock_sigmoid, mock_dataloader, mock_model, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.sigmoid")
    def test_run_inference_uses_correct_threshold(
        self, mock_sigmoid, mock_dataloader, mock_model, mock_device
    ):
        ...
        threshold = 0.7
        probas, classes = run_inference(
            mock_dataloader, mock_model, mock_device, approval_threshold=threshold
        )
    ...
    @patch("src.inference.prepare_inference_data.torch.sigmoid")
    def test_run_inference_model_in_eval_mode(
        self, mock_sigmoid, mock_dataloader, mock_model, mock_device
    ):
    ...
    def test_run_inference_uses_no_grad_context(
        self, mock_no_grad, mock_dataloader, mock_model, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.sigmoid")
    @patch("src.inference.prepare_inference_data.logger")
    def test_run_inference_logs_info_messages(
        self, mock_logger, mock_sigmoid, mock_dataloader, mock_model, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.sigmoid")
    def test_run_inference_handles_single_sample(
        self, mock_sigmoid, mock_model, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.sigmoid")
    @patch("src.inference.prepare_inference_data.logger")
    def test_run_inference_handles_exceptions(
        self, mock_logger, mock_sigmoid, mock_device
    ):
        """Verifica tratamento de exceções."""
    ...
    @patch("src.inference.prepare_inference_data.torch.sigmoid")
    def test_run_inference_correct_probability_range(
        self, mock_sigmoid, mock_dataloader, mock_model, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.sigmoid")
    def test_run_inference_classes_are_binary(
        self, mock_sigmoid, mock_dataloader, mock_model, mock_device
    ):
    ...
    @patch("src.inference.prepare_inference_data.torch.sigmoid")
    def test_run_inference_default_threshold_half(
        self, mock_sigmoid, mock_dataloader, mock_model, mock_device
    ):
